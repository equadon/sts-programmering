...
public class SnookerInitializer implements BallInitializer {
    ...
    public List<PoolBall> createBalls(Table table) {
        ...
        balls.add(BallFactory.createSnookerBall(2, table, position, radius));

        // Green ball
        position = new Vector2D(center.x - 0.15 * table.width, yLine);
        balls.add(BallFactory.createSnookerBall(3, table, position, radius));

        // Brown ball
        position = new Vector2D(center.x, yLine);
        balls.add(BallFactory.createSnookerBall(4, table, position, radius));

        // Blue ball
        position = new Vector2D(center.x, center.y);
        balls.add(BallFactory.createSnookerBall(5, table, position, radius));

        // Pink ball
        position = new Vector2D(center.x, radius/2.0 + table.height / 3.0);
        balls.add(BallFactory.createSnookerBall(6, table, position, radius));

        // Black ball
        position = new Vector2D(center.x, 0.125 * table.height);
        balls.add(BallFactory.createSnookerBall(7, table, position, radius));
