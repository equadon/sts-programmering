...
class Board {
    ...
    private Board.Hill[] hills = {};          // The hills. This defines the board topography
    ...
    private Board.Position topLeft, bottomRight;            // initial piece positions
    ...
    private ArrayList<Board.Region> debugRegions;                                     //   of controlled areas. Turn on at own risk.
    ...
    static class Position implements Serializable {
        ...
        int distance(Board.Position p)    // distance is geometric
        {
        ...
        boolean equals(Board.Position p) {
    ...
    {
        private Board.Position pos, center;                          // its position (upper left corner of bounding box) and center
        ...
        Hill(Board.Position pos, int diameter, int rings) {
            this.pos = pos;
            this.diameter = diameter;
            this.rings = rings;
            radius = diameter / 2;                          // radius is half diameter
            height = 1.1f + ((float) rings) / 10;            // height is 1.1 + 0.1*number of rings
            center = new Board.Position(pos.x + radius, pos.y + radius); // calculate center
        }

        //-------------- invalid: returns true if the hill is invalid

        boolean invalid(Board.Hill[] hills, Area boardArea)     // params are the hills generated so far
        {
            boolean r = false;
            ...
            for (Board.Hill otherHill : hills)                             // Ow, for all hills generated so far
            {
    ...
    Board(boolean isLite, boolean withHills, int graphics, int shape, int boardS, int height,                   // params are board dimensions
          int width, int pebbles, boolean iPlayWhite)          // and initial number of pebbles
    {
        ...
        if (circleBoard)                                              // If a circular board
        {
            ...
            topLeft = new Board.Position(SIZE * (WIDTH + 2) / 2, SIZE + 1);          //   the initial piece positions are here
            bottomRight = new Board.Position(SIZE * (WIDTH + 2) / 2,             //   the top and bottom positions of the ellipse
                    SIZE * (HEIGHT + 1) - 1);
        } else                                                          // If a rectangular board
        {
            ...
            topLeft = new Board.Position(SIZE, SIZE);                       //    initial positions are
            bottomRight = new Board.Position(SIZE * (WIDTH + 1) - 1,            //    opposite corners of the board
                    SIZE * (HEIGHT + 1) - 1);
        }
        ...
        initialselectors[1] = newPiece(Piece.Type.RUBBLE, true, true, new Board.Position(selectorPosX, 3 * SIZE));
    ...
    Board.Hill[] generateHills() {
        Random rand = new Random();                        // get me a new random number generator
        Board.Hill hill;                                         // local hill. I'll generate a hill there and
        ...
        hills = new Board.Hill[numberOfHills];                  // get the array for hills
        for (int i = 0; i < numberOfHills; i++)              // for each element in the array,
        {
            do {
                ...
                hill = new Board.Hill(new Board.Position(x, y), d, rings);    //   and create this hill
    ...
    void setHills(Board.Hill[] hills)                            // just store the parameter in hills
    {
        this.hills = hills;
    }
    ...
    boolean isLegal(Board.Position p) {
    ...
    float elevation(Board.Position pos) {
        for (Board.Hill hill : hills)                           // for every hill
        {
            if (hill.onIt(pos)) {
    ...
    Board.Position realPosition(int x, int y, Piece exemptPiece)           // params are pixel coordinates and
    {                                                                         // the piece exempt from proximity check
        Board.Position res = new Board.Position(x, y);                                    // First create the proper position
        {
    ...
    Board.Position realPosition(int x, int y) {
    ...
    final Piece newPiece(Piece.Type pt, boolean w, boolean my, Board.Position p) {
    ...
    boolean near(Piece.Type pt, Board.Position pos) {
        boolean found = false;
    ...
    boolean iControl(Board.Position pos) {
        return myArea.contains(pos.x, pos.y);
    }

    boolean opponentControls(Board.Position pos) {
    ...
    boolean canMoveTo(Board.Position pos) {
        return highlightArea.contains(pos.x, pos.y);
    }

    boolean canBuildPebbleOn(Board.Position pos) {
...

    {
        ...
        if (allVisible)                                                        // if allVisible then draw both white and black Pebbles
        {
            for (int i = 1; i <= remaining(true); i++) {
                myPebble.draw(g, new Board.Position(i * SIZE + SIZE / 2, SIZE / 2), Color.white);
            }
            for (int i = 1; i <= remaining(false); i++) {
                myPebble.draw(g, new Board.Position(i * SIZE + SIZE / 2, SIZE * (HEIGHT + 1) + SIZE / 2), Color.black);
            }
        } else                                                                  // else only show my Pebbles
        {
            for (int i = 1; i <= remaining(iPlayWhite); i++) {
                myPebble.draw(g, new Board.Position(i * SIZE + SIZE / 2, SIZE / 2), iPlayWhite ? Color.white : Color.black);
            }
        }
        ...
        if (debug) {
            ...
            for (Board.Region region : debugRegions) {
                g.draw(region.area);
            }
        }
...

    {
        ...
        if (!waiting & !playBackBoard) {
            myPebble.draw(page, (new Board.Position(SIZE / 2, SIZE * (HEIGHT + 1))), Color.red);
        }
    ...
    private class Region {
        ...
        Board.Position center;          // center of the bounding box
        ...
        private void setBounds() {
            ...
            center = new Board.Position(boundingBox.x + boundingBox.width / 2, boundingBox.y + boundingBox.height / 2);
            radius = center.distance(new Board.Position(boundingBox.x, boundingBox.y));
        }
        ...
        private void addMe(ArrayList<Board.Region> regList, ArrayList<Piece> pieces) {
        ...
        private void checkPiece(Piece p, ArrayList<Board.Region> newRegions, ArrayList<Piece> pieces) {
            ...
            else if (p.rangeArea.intersects(boundingBox))        // else, if the bounding box intersects the range
            {
                ...
                else if (!newArea.isEmpty())                        //  else, if that turns out to be nonempty
                {
                    ...
                    Board.Region newRegion = (new Board.Region(newArea,        // and create a new region for the intersection
...

    {
        ...
        ArrayList<Board.Region> regions = new ArrayList();                                 // initialise the "regions" to contain just one,
        regions.add(new Board.Region((Area) boardArea.clone(), 0, allVisible));               // the entire board with control 0
        ...
        {
            ArrayList<Board.Region> newRegions = new ArrayList();                           //  the  regions that need to be examined after this
            for (Board.Region thisRegion : regions)      // for each existing region
            {
        ...
        while (!opponentPieces.isEmpty())                   // for each of opponent's pieces
        {
            ...
            ArrayList<Board.Region> newRegions = new ArrayList();                             // reset the new regions to be generated
            for (Board.Region thisRegion : regions)                        // for each region
            {
        ...
        for (Board.Region thisRegion : regions) {
